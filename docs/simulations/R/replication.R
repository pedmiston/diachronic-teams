...
timeline_plot <- ggplot(BotsReplication) +
  ...
  scale_color_manual(values = totems_theme$color_picker(c("green", "orange"))) +
...
BotsReplicationFinal <- BotsReplication %>%
  filter_final_round() %>%
  mutate(strategy_rev = factor(strategy, levels = c("solo", "full")))

final_plot <- ggplot(BotsReplicationFinal) +
  ...
  geom_point(aes(color = strategy), position = position_jitter(0.3),
             shape = 1) +
  scale_x_discrete("Strategy", labels = c("Isolated", "Synchronic")) +
  totems_theme$scale_y_inventory_size +
  scale_color_manual(values = totems_theme$color_picker(c("green", "orange"))) +
  scale_fill_manual(values = totems_theme$color_picker(c("green", "orange"))) +
  coord_cartesian(ylim = c(6, 16)) +
  totems_theme$base_theme +
  theme(
...
gridExtra::grid.arrange(
  crotchet::read_graphviz("calendar-hours", "totems"),
