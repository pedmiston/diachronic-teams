...
author-information: |
    ...
    \And {\large \bf Maxime Derex} \\ \texttt{maxime.derex@gmail.com} \\ Biosciences Department \\ University of Exeter
...
abstract: |
  ...
  other technology that were created by others like no other species of animal.
  But what is the impact of inheritance---starting off a problem solving task
  with partial solutions inherited from a previous generation---on future
  problem solving ability? Does inheritance provide a reliable shortcut to
  individual learning without affecting underlying problem solving ability, or
  does it affect the way in which future problems are solved? To begin to
  answer these questions, we used an iterated learning paradigm to measure the
  effectiveness of problem solving over four generations of inherited
  solutions, where the solutions of one generation of problem solvers were
  passed on to be inherited by an individual in the next generation in a form of
  teamwork we refer to as diachronic collaboration. We find that through four
  generations of diachronic collaboration, problem solvers were consistently
  able to solve more problems in a single session than their ancestors, even as
  problems became more difficult to solve. We investigate whether the size of
  the inheritance had an impact on future problem solving ability, and find
  mixed results, ultimately concluding that the impact of inheritance on
  problem solving can only be determined relative to alternative strategies for
  solving the same set of problems but without inheritance. Although more work
  on this topic is required, we believe extending the iterated learning
  experimental paradigm to answer questions about iterated problem solving is
...
chunks <- c("diachronic-collaboration")
...
to solve the problems necessary for survival in diverse habitats [@Boyd:1985cu;
@Richerson:2008:not; @Henrich:2015srt; @Boyd:2017diff]. As problem solvers,
humans are particularly adept at using, adapting, and, in some cases, improving
...
problem solvers. The purpose of this research is to understand why diachronic
collaboration may be crucial to human cultural evolution by assessing its
effectiveness as a problem solving strategy.

```{r fig1, engine="dot", fig.env="figure*", fig.pos="b", fig.width=6, fig.height=2, fig.cap="In diachronic collaboration, the product of one generation of problem solving is inherited to be used and improved by the next generation of problem solvers. The ability to engage in diachronic collaboration---to improve something that was created by someone else---is required in order to observe the ratcheting of technological complexity in a culture."}
```

Inheritance in a problem solving context is made possible by social learning
abilities, or what @Thorndike:1898 called "from an act witnessed, learn to do
an act" (p. 50). Thorndike's failed experiments on social learning in chicks,
cats, and dogs made him and others skeptical that social learning was
a widespread phenomenon among animals, but recent evidence has supplanted this
view, identifying a growing number of species that appear to inherit
behaviors from the observation of others. Some of the evidence comes from
identifying what appear to be cultural traditions in natural populations, and
ruling out explanations for these behaviors not having to do with social
learning, such as environmental conditions constraining the behaviors
individuals might learn on their own and not through social learning. This
approach has identified cultural traditions in primates, monkeys, cetaceans,
and birds. Lab experiments have demonstrated social learning abilities in some
of these species and others, including rats, fish, and even some insects. As
a result, what separates the mechanisms of inheritance in humans from other
animals is not qualitative, but quantitative.
...
```{r tab1, results="asis"}
Exp1Participants %>%
  xtable::xtable(caption = "Participants in the Experiment.") %>%
...
Once a tool was discovered, the recipe for its creation was recorded in the History panel (Fig. 2A). Participants could review their history and see the recipes for their previous discoveries, even items they had discarded. The History panel was also
how diachronic inheritance was implemented. Diachronic problem solvers after the
first generation began the problem solving task with an additional tab in the
History panel that, when selected, listed all of the recipes for the items
created by the previous generation.

```{r fig2, fig.width=8, fig.env="figure*", fig.cap="The Totems game. Left. The Totems gameplay interface. Participants generated guesses by dragging items into the Workshop and selecting the Try button. If the guess created an item, it could be dragged into the Stock panel, and used again. Once segments of a totem were made through cutting, carving, and painting, they could be dragged into the Totem panel for scoring. Right. A sample of the solution landscape. The top row of 6 items are the initial resources available to problem solvers at the start of the session. These resources must be combined to build the first generation of tools, including a refined rock, a club derived from an antler, and a small branch of a tree. The first 6 generations of possible tools are shown. The axe is required to construct the first totem pole."}
grid.arrange(
...
innovations discovered by their predecessors (Fig. 3).
...
```{r fig3, fig.env="figure", fig.cap="Number of innovations discovered by each generation. Each of the thin green lines is a team. Means in each generation are shown as green X's. The thick blue line is the model predictions with ±1 standard error."}
...
Fig. 5 shows the change in accumulated difficulty score based on the number of innovations
...
Diachronic problem solvers who inherit solutions must start the
...
inherited (Fig. 6A). We refer to to this stage
...
`r report_lm_mod(learning_rates_mod, "InheritanceSize")` (Fig. 6B),
...
```{r fig6, fig.width=6, fig.height=4, fig.env="figure*", fig.cap="Learning times. A. The distribution of learning times, with outliers highlighted. B. Linear relationship between inheritance sizes and learning times. Outliers excluded from the regression are shown as X's."}
grid.arrange(
