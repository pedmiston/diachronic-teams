...
abstract: |
  ...
  limitations of thiswork, and motivate future directions.
...
instruction and imitation that provide reliable ways of transfering problem
...
and improve the tools and other innovations of others. The
...
Looking at problem solving performance relative to the number of tools that
were inherted rather than the generation of the participant revealed similar
results (Fig. 4). As the number of inherited tools increased, the number of new
tools discovered decreased, `r exp1$inheritance_size_slope_stats` (Fig. 4B).
As expected from the analysis of performance over generations reported above,
the ability of participants to exceed their ancestors decreased with larger
inheritances. However, when tools are weighted by combinatorial complexity,
the amount by which participants are able to exceed their ancestors actually
increases for larger inheritance sizes, `r exp1$delta_diff_slope_stats` (Fig.
4C), though we are hesitant to draw strong conclusions from this finding as it
may be explanable by the characteristics of the solution landscape, a point we
...
solving knowledge would accumulate given that problem solving knowledge could be
