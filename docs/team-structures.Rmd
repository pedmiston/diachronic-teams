...
```{r intro, include=FALSE, cache=FALSE}
```
...
```{r methods, include=FALSE, cache=FALSE}
```
...
to the next generation. All diachronic participants after the first generation
started off the problem solving task with some solutions that had been
discovered by a previous generation.

Participants assigned to the synchronic team structure worked independently and
in parallel but their labor was coordinated in that solutions discovered by one
team member were immediately shared with the other members of the team who were
working at the same time. Isolated participants completed the problem solving
task as individuals, only benefiting from the solutions they could discover on
their own.

(ref:team-structures-all) All experimental conditions. Participants assigned
to diachronic teams worked one at a time, each inheriting the solutions
...
(ref:team-structures-exp3) Problem solving strategies compared in Experiment 2.

```{r team-structures-exp3, engine="dot", out.width="70%", fig.cap="(ref:team-structures-exp2)"}
```

# Experiment 3 {-}

```{r teamsize, include=FALSE}
```

(ref:team-structures-exp4) Problem solving strategies compared in Experiment 3.

```{r team-structures-exp4, engine="dot", out.width="80%", fig.cap="(ref:team-structures-exp3)"}
```
