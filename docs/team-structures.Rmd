...
have the most of them. Human tool users are particularly skilled at using and
improving tools and other solutions to problems that were first discovered by
other people, as evidenced by the widespread prevalence of reuse in the history
of human technology [@Basalla]. The reuse 

observation of and interaction with others, generally known as social learning.
Humans are exceptional social learners, having evolved a number of abilities
such as imitation, teaching, and language that enable the sharing of tools and
other solutions to problems with other problem solvers. As a result of our
social learning abilities, human cultures have been able to accumulate complex
tools that no individual could invent on their own.

Cultural inheritance involves both vertical and horizontal forms of
transmission [@Boyd:1985cu]. Vertical transmission involves passing cultural
products from parent to child, whereas in horizontal transmission, cultural
products are passed among peers in the same generation. Although both forms of
cultural transmission involve social learning, the consequences of either
vertical or horizontal transmission have not been directly compared. Here we
tested whether problem solving knowledge that is inherited at the start of a
problem solving task via vertical transmission results in different problem
solving performance than purportedly the same knowledge shared horizontally
among problem solvers working in parallel.

Previous research on the difference between vertical and horizontal
...
Whether or not individuals learn from parents or peers may also have
...
region, and found two distinct distributions. In one, the projectile point
designs were highly correlated with one another, indicating a culture of horizontal
transmission, where members of the society copied the designs of the most
prestigious hunters. In a different region, over the same period, the recovered points
were very dissimilar from one another, indicating a process of individual
learning and vertical transmission in which each family may have innovated
their own point designs. This interpretation has been supported by experimental
and computational simulations showing that fully connected networks of problem
solvers result in greater convergence and smaller variation in design than
...
known as "theory of mind", is by most formulations independent of who is being
reasoned about.

However, there is a difference between vertical and horizontal transmission
that has yet to be explored, and that is the difference in learning experience
between traits that are passed from parent to child, and traits that
are shared among individuals working in parallel. When solutions are passed
vertically, the new generation of problem solvers begins with the solutions
created by the previous generation.

We argue that the difference between vertical and horizontal transmission is
in the unique learning experience afforded by vertical transmission. In vertical
transmission, individuals start off with solutions to problems that were
inherited from a previous generation. These solutions provide a unique learning
environment as compared to solving purportedly the same problems without
...
guessing efficiency on a per-tool basis. Did strategy improve guessing
...
there was more than one adjacent tools, each incorrect guess was counted toward
...
classified each guess into one of four types, shown in Table
...
Looking just at redundant guesses, we found that second generation
...
Looking just at unique guesses, we found no difference between
...
inheritance---inheriting from someone else---to the performance of isolated
...
recreating inherited tools before extending these to discover new tools.
...
tools. We refer to this portion of the experiment we refer to as the learning period. We
