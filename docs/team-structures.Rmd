...
Humans are not the only tool users in the animal kingdom, but we do
seem to use the most of them. What sets human tool users apart
is the ability to use tools and other solutions to problems that were created by other people,
which is a form of social learning. Social learning is learning
new skill from the observation or interaction with someone else
Humans have evolved a system of cultural inheritance that allows groups of
problem solvers to solve problems that individuals would be unlikely to solve
on their own [@Henrich:2015; @Boyd:2017diff; @Derex:2015cbb]. Human cultural
inheritance is enabled by social learning abilities that a
to cultural inheritance 

Cultural
inheritance involves both vertical and horizontal forms of transmission
[@Boyd:1985cu]. Vertical transmission involves passing cultural products from
...
horizontal transmission have not been compared. Here we tested whether problem
solving knowledge that is inherited at the start of a problem solving task via
vertical transmission results in different problem solving performance than
purportedly the same knowledge shared horizontally among problem solvers
working in parallel.
...
region and found two distinct distributions. In one region, the projectile point
...
were very dissimilar from one another, indicating a process of vertical
transmission in which each family may have innovated their own point designs.
...
traits in a population, it is not generally considered to have an impact on the
types of problem solving behaviors that can be performed by individuals in each
