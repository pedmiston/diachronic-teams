...
We found that the major difference between diachronic and isolated
...
(ref:diachronic-v-isolated-inheritance) **A.** Learning times by strategy. Learning time refers to
the number of minutes it took to recreate the inherited items. For diachronic
participants (**left**), learning time scaled linearly with generations, but
for isolated participants returning for multiple sessions (**right**), learning
times were stable across sessions. **B**. Number of guesses required to
discover the first new tool each session.
...
We compared the effectiveness of problem solving strategies involving vertical
and horizontal transmission in order to better understand the processes that
give rise to cumulative cultural evolution. In vertical transmission, solutions
created by one generation are inherited at the start of the problem solving task
by the next generation, whereas in horizontal transmission, all problem solvers
start at the same time, and collaborate in parallel.

Although mode of transmission is known to have an impact on the distribution of
traits in a population, it has not been considered to have an impact on the
problem solving performance that can be performed by individuals in each
group. Part of the reason is that both horizontal and vertical transmission are
believed to involve the same social learning mechanisms. This makes the
distinction between vertical and horizontal cultural transmission different
...
being copied. For example, the ability to reason about other people's mental states,
known as "theory of mind", is largely independent of who is being reasoned about.
