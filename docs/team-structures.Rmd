...
Humans are not the only tool users in the animal kingdom, but we do seem to
have the most of them. What sets human tool users apart may not be our ability
to make tools, but our ability to use and improve tools that were first created
by other people. This ability is often referred to as the "ratchet effect"
[@Tomasello:1993vh; @Dean:2013db], and is believed to be an important component
of cumulative cultural evolution. One of the defining features of cumulative
cultural evolution is that cultures of problem solvers, through generations of
ratcheting, can sustain tools and other solutions to problems that no individual
could recreate on their own [@Boyd:2011gp]. Although the capacity to engage in
cultural ratcheting is well-known, the effectiveness of cultural transmission
as a strategy for solving problems is an area in need of further research. The
purpose of this research was to better understand the effectiveness of cultural
transmission in accumulating solutions to problems that no individual could
...
transmission have not been directly compared. Here we tested whether problem
solving knowledge that is inherited at the start of a problem solving task via
vertical transmission has a different impact on problem solving performance
than purportedly the same knowledge shared horizontally among problem solvers
working in parallel.
...
@Boyd:1985cu]. Whether or not individuals learn from parents or peers may also
...
groups of problem solvers result in smaller variation in design than than
...
@Derex:2013cbb had groups and individuals complete the same problem solving
...
working alone [@Acerbi:2016ej]. As a result, horizontal transmission is
believed to be adaptive for a wider range of problem solving scenarios than
individual learning [@Golman:2010fz; @Rendell:2010go].

When vertical transmission has been studied directly, it has not been compared
to alternative strategies for solving the same problems. For example,
@Caldwell:2008gf had participants create paper airplanes or spaghetti towers in
a transmission chain design, where new generations of problem solvers learned
from the previous generation. The results showed that performance tended to
improve along the transmission chains, suggesting that human problem solvers
are uniquely skilled at using and improving solutions to problems that were
created by others. But since the performance of the transmission chains was not
compared to performance of isolated individuals given the same opportunities,
we do not know whether vertical transmission by itself can result in the
accumulation of problem solving knowledge that no individual could recreate on
their own.
...
generation. In **synchronic collaboration**, participants work on the same
problem solving task at the same time and in parallel, sharing solutions with
each other as they are discovered. Both diachronic and synchronic strategies
are compared to an isolated strategy in which individuals work alone on the
same problems.
...
that diachronic collaboration is similarly effective to the alternative
strategies, but we also demonstrate the ways in which starting off a problem
solving task with solutions inherited from a previous generation results in
a unique form of problem solving compared to the alternative strategies. In
Experiment 2, we compared four generations of diachronic problem solvers to
four sessions of isolated problem solving in order to tease apart the effects
of inheriting from someone else as opposed to inheriting from oneself. We find
that an isolated strategy results in better final performance, but not any more
efficient guessing. In fact, we find that diachronic collaboration may prove
effective in relieving the costs of fixation often incurred by individuals
working alone. In Experiment 3, we investigated how the effectiveness of
diachronic and synchronic collaboration is affected by increasing group size.
Here we find that diachronic collaboration scales better as more people are
added as opposed to synchronic collaboration, in which teamwork may become more
inefficient with larger groups.
...
```{r team-structures-all, out.width="100%", fig.cap='(ref:team-structures-all)'}
...
very few resulted in new tools. For example, of all the combinations that could
...
review their past discoveries by clicking on a tool to see its recipe.
...
reported in multiple experiments. Specifically, data from first and second
...
teams is reported in Experiment 1 and Experiment 3.
...
guessing efficiency on a per-tool basis. Did team strategy improve guessing
efficiency while controlling for tool? Finally, we compared the guessing
...
teams discovering more tools than isolated individuals, `r exp1$S2_v_I50`. This
result, showing an advantage of teamwork over individual labor, suggests that
maximizing individual learning time is not always the most effective way to
...
discovered by participants in conditions without inheritance,
...
tool. The "No inheritance" condition includes guesses from synchronic
...
a difference between second generation diachronic
...
Redundant guesses are unhelpful in that they will never
...
when labor hours are controlled.
...
In Experiment 2, we compared the performance of four generations of vertical
transmission---inheriting from someone else---to the performance of isolated
individuals given four separate sessions (Fig.
...
that isolated individuals given four sessions were able to accumulate more
tools than diachronic teams given the same total amount of time. The remaining
analyses test hypotheses about why an isolated strategy was more effective than
a diachronic strategy. In investigating why an isolated strategy is more
...
Participants who inherited an innovation record from a previous generation
or from a previous session took on average `r exp2$mean_learning_time` minutes of the 25
...
We found that a large difference between diachronic and isolated
individuals was in the length of the learning period.
Isolated individuals returning for multiple sessions took significantly
...
\@ref(fig:diachronic-v-isolated-inheritance)A).

This result means that what made an isolated strategy successful in
...
However, the benefit of an isolated strategy in improving the time to
recreate existing tools comes at the cost of becoming fixated on previous
and incorrect learning experiences. Next we report the cost of generating
the first new discovery each new session.
...
(ref:diachronic-v-isolated-inheritance) **A.** Learning times by strategy.
Learning time refers to the number of minutes it took to recreate the inherited
items. For diachronic participants (**left**), learning time scaled linearly
with generations, but for isolated participants returning for multiple sessions
(**right**), learning times were stable across sessions. **B**. Number of
guesses required to discover the first new tool each session. For diachronic
participants (**left**), new tools were equally likely to be discovered by each
new generation, but for isolated participants (**right**) the cost of
discovering a new item increased with new sessions.
...
than diachronic participants. Instead, isolated participants, given multiple sessions,
...
We did obtain evidence of one way in which a diachronic strategy can promote a
more efficient form of problem solving. We found that isolated individuals were
more likely to become fixated on previous, incorrect solutions, causing them to
guess inefficiently. These fixation effects extended over multiple sessions,
causing the performance of some isolated individuals to stagnate. Diachronic
participants, however, were less impacted by fixation, and were more easily
able to move beyond the solutions they inherited to discover new solutions.
These results reveal a novel benefit to vertical transmission in relieving
the pressures that slow individual problem solvers down.
...
can solve problems that no individual could on their own. Previous research
addressing this question has found that groups of problem solvers collaborating
via horizontal transmission can in some cases raise the performance of the group
beyond what would be likely to be achieved individually. We extended this line
of research by comparing the effectiveness of vertical transmission, where
individuals start off a problem solving task by inheriting directly from a previous
generation.

We found that vertical transmission promotes a unique kind of problem solving
compared to horizontal transmission and isolated individuals. In comparing
vertical and horizontal transmission, we found that the effectiveness of
vertical transmission is more likely to scale with additional problem solvers
compared to horizontal transmission. In comparing vertical transmission
to isolated labor, we found that vertical transmission may be helpful in
remediating the costs of fixation that slow down or hamper progress by
isolated learners.
...
diachronic and synchronic elements. Among animals, for example,
multiple forms of cultural transmission are observed occurring in the
wild, and not all of these were tested here [@Whiten:2008dl].

Another difficulty in interpreting our results as evidence of a difference
between vertical and horizontal transmission is that both forms of cultural
...
Nonetheless, our results suggest that there may be a difference between horizontal
and vertical transmission. The reason is related to the unique learning
environment experienced by diachronic collaborators. In vertical transmission,
new generations directly inherit and adapt solutions at the start of a problem solving
task, making continued problem solving dependent on these previous learning
experiences. These learning experiences may be different than acquiring
purportedly the same knowledge after initial learning has already taken place.
What this means is that vertical transmission may promote unique problem
solving behaviors that could not be achieved individually.

In seeking to explain how and why humans are unique skilled problem solvers,
many have pointed to our ability to copy skilled behaviors learned by others.
In modern societies, we clearly see the results of copying and improving
behaviors in that some cultural artifacts are very complex and would be unlikely
to be rediscovered by individuals working alone. But whether we can detect
the impact of social learning on individual problem solving performance is
an open area of research. Here we demonstrate that vertical transmission
is a unique form a problem solving that is worth further investigation.
