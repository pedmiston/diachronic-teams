...
```{r}
player_trajectories <- Guesses %>%
  group_by(PlayerID, InventorySize, NumAdjacent) %>%
  summarize(Guesses = n()) %>%
...
(trajectory_plot %+% filter(player_trajectories, Strategy == "Isolated")) +
  scale_color_manual("Strategy", values = t_[["isolated_color"]])
```
...
(trajectory_plot %+% filter(player_trajectories, Strategy == "Diachronic")) +
  ...
  scale_color_manual("Strategy", values = t_[["diachronic_color"]])
```
...
(trajectory_plot %+% filter(player_trajectories, Strategy == "Synchronic")) +
  ...
  scale_color_manual("Strategy", values = t_[["synchronic_color"]])
```
