...
abstract: |
  ...
  generation save time in future problem solving in proportion
  to the cost of creating the inheritance, or does it affect
  ...
  the three strategies while controlling for total labor hours, so that the
  consequences of inheritance can be interpreted relative to the cost of
  creating it, and relative to the costs of not using an alternative problem
  solving strategy.
csl: "theme/apa.csl"
...
[@Basalla:1989wq; @Valverde:2007fs] [cf. @Borner:2004dm]. But what do
innovators and other problem solvers gain from inheriting solutions created by
others? Do problem solvers gain a head start on future problem solving, or does
inheriting an existing solution to a problem affect the way in which future
problems are solved? We attempt to answer this question by studying the
problem solving ability of individuals who inherit solutions created by
a previous generation at the start of a problem solving task. This type of
collaboration, organized around inheritance, is referred to as **diachronic
collaboration** (Fig. \@ref(fig:team-structures)). To understand how
inheritance impacts problem solving ability, we compared the effectiveness of
diachronic collaboration to two alternative strategies for solving the same
set of problems in the same amount of time but without diachronic inheritance:
...
individual learning (Fig. \@ref(fig:team-structures)).
...
Diachronic collaboration encapsulates an important process in human cultural
evolution: the iterative improvement of tools and other solutions to problems
over generations of problem solving.

Diachronic collaboration is enabled by social learning mechanisms that allow
problem solvers to learn from previous generations of problem solvers rather
than striving to solve the same problems independently. Models of the cultural
evolutionary process demonstrate that social learning is adaptive relative to
individual learning when the environment is stable and individual learning is
costly. A population of smart individuals who are unable to learn from each
other can only accomplish so much in a single lifetime, and their
accomplishments are not inherited by any future problem solvers, and must be
rediscovered independently, and redundantly by others. A population of
individuals _with_ social learning can save time and shortcut individual
learning by learning a costly skill from someone else. These shortcuts
accumulate, and eventually populations of social learners are able to
accomplish as a group what no individual could have accomplished on their own.

Boyd and Richerson [-@Boyd:1983; @Richerson:2005] argue that the success of our species in solving
the problems necessary for survival in nearly every terrestorial environment on
the planet arises from a predisposition to imitate others, even when we
may not fully understand the reasons fo or implications of repeating a particular behavior. @Tennie:2009 argue that humans are unique as replicators
of behaviors because we can inherit solutions to problems from outside
our "zone of latent solutions". 
...
solving over four generations of diachronic problem solving (Fig.
\@ref(fig:team-structures-exp1)). We answer the following questions about
the achievements of diachronic teams. First, we determine whether and by how
...
participant was randomly assigned to a single generation in a single team.
Participants in generations 2 through 4 began the problem solving task with the
solutions that were discovered by the previous generation. The unique
innovations of participants in generations 1-3 were inherited by one (and only
...
This result was obtained using Page's trend test, which is test of
monotonicity for repeated measures. We used this test to measure whether
...
We next measured the linear increase in innovations by generations by fitting
