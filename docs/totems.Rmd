...
```{r types-of-time, fig.width=8, fig.height=3, fig.cap="**Left panel.** Three strategies for allocating the same number of labor hours. The other panels show the differences between these strategies along different time dimensions. **Middle.** Relationship between labor hours and calendar hours. Labor hours are being controlled across the strategies, which forces diachronic and synchronic teams to differ in calendar hours. The rate at which labor hours are spent (per calendar hour) is the same for diachronic teams and isolated individuals. **Right.** Differences between teams in terms of time for individual learning. Isolated individuals have the most time for individual learning."}
```
...
\begin{equation}
problem\ solution = inheritance + ability \cdot time\\
problem\ solution = inheritance \cdot ability \cdot time\\
\end{equation}
