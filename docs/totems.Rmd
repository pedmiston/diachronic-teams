...
```{r team-structures-50min}
draw_graphviz("team-structures-50min", "totems")
```
...
grid.arrange(
  read_graphviz("team-structures-50min", "totems"),
...
final_num_innovations_50_mod <- lmer(
  NumInnovations ~ DG2_v_DG1 + DG2_v_I50 + DG2_v_S2 + (1|TeamID),
  data = PlayerPerformance50min
)
tidy(final_num_innovations_50_mod, effects = "fixed") %>%
  kable(caption = "Final number of innovations model (50 min).")
```

### Teamwork versus individual labor

```{r teamwork-v-individual-50-mod, echo=1, results="asis"}
teamwork_50min_mod <- lmer(
  NumInnovations ~ DSvI + DvS + (1|TeamID),
  data = filter(PlayerPerformance50min, !(Strategy == "Diachronic" & Generation == 1))
)
tidy(teamwork_50min_mod, effects = "fixed") %>%
  kable(caption = "Teamwork model (50 min). Tests whether teams (Diachronic and Synchronic) discovered more innovations than isolated individuals in a single 50 minute session.")
```
...
grid.arrange(
  ...
  num_innovations_over_time_50 +
    aes(CalendarTime) +
    t_$scale_x_calendar_time_50 +
    theme(legend.position = c(0.12, 0.75)),
  num_innovations_over_time_50 +
    aes(PlayerTime) +
    t_$scale_x_player_time_50 +
...
```{r team-structures-type-of-inheritance}
draw_graphviz("type-of-inheritance", "totems")
```

## Inheritance compared to repeating sessions

```{r type-of-inheritance}
```

```{r num-innovations-by-generation-50-fig, fig.width=8, fig.cap=""}
grid.arrange(
...
## Impact of inheritance on future problem solving

```{r diachronic-inheritance}
```

## Cost per innovation

```{r load-axis-pics, include=FALSE}
```

```{r cost-per-item}
```

```{r guesses-per-item-50-fig, fig.width=8}
grid.arrange(
  guesses_per_item_50_plot,
  guesses_per_item_50_mod_plot,
  nrow = 1,
  widths = c(0.7, 0.3)
)
```

```{r guesses-per-item-by-discovery-50-fig, fig.width=8, fig.height=6}
grid.arrange(
  guesses_per_item_50_by_discovery_plot,
  guesses_per_item_50_by_discovered_mod_plot,
  nrow = 1,
  widths = c(0.7, 0.3)
)
```

## Labor time per innovations

```{r time-per-item-50-fig, fig.width=8}
grid.arrange(
  time_per_item_50_plot,
  time_per_item_50_mod_plot,
  nrow = 1,
  widths = c(0.7, 0.3)
)
```

```{r time-per-item-by-discovery-50-fig, fig.width=8, fig.height=6}
grid.arrange(
  time_per_item_50_by_discovery_plot,
  time_per_item_50_by_discovered_mod_plot,
  nrow = 1,
  widths = c(0.7, 0.3)
)
```
...
```{r guesses-per-item-100-fig, fig.width=8}
grid.arrange(
  guesses_per_item_100_plot,
  guesses_per_item_100_mod_plot,
  nrow = 1,
  widths = c(0.7, 0.3)
)
```

```{r guesses-per-item-by-discovery-100-fig, fig.width=8, fig.height=6}
grid.arrange(
  guesses_per_item_100_by_discovery_plot,
  guesses_per_item_100_by_discovered_mod_plot,
  nrow = 1,
  widths = c(0.7, 0.3)
)
```
