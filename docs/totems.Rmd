...
```{r performance-over-time, fig.height=6, fig.width=8}
ggplot(player_trials) +
  aes(TeamTime, InventorySize, group = PlayerID, color = StrategyLabel) +
  geom_line(alpha = 0.2) +
...
```{r unique-guesses-over-time}
ggplot(totems_workshops) +
  aes(TeamTime, TeamNumUniqueGuesses, group = ID_Group_Time, color = StrategyLabel) +
  geom_line(alpha = 0.2) +
