...
A tool or other solution to a problem that required a lot of work to create
benefits its user by saving them
...
learning. Models of the cultural evolutionary process suggest that inheriting from others
via social learning is more adaptive than individual learning when individual
learning is costly in a stable environment [@Boyd:1988slm].

At the scale of human cultures involving thousands of generations of
...
in groups of 2 or 4 individuals, each participating for 25 minutes.
...
players to iteratively solve problems involving combining items to create new
items (tools) for the purposes of building "a sacred totem to appease the gods."
...
To generate new guesses, participants tried combinations of items by dragging
items into a workshop panel (Fig. \@ref(fig:interface)). Of all the valid
combinations of items that can be made (combinations of between 1 and 4 items,
with replacement), only a small percentage successfully yielded new items. The 6 initial
resources can be combined with between 1 and 4 other items with replacement for
a total of `r count_unique_guesses(6)` unique guesses, only three of which yield
any new tools: a branch can be broken off a tree, an antler refined into a club,
and two rocks can be combined to yield a shaped stone. If the guess
yielded an item, it could be stored in a stock panel. A maximum of 12 items
could be stored in the stock panel. Items could be discarded if space was needed
in the stock panel for new items, or if an item was not needed for any reason.
...
The tab displayed the current score of each player, so synchronic were
notified that a discovery had been made by seeing their teammates score rise
above their own.
...
remaining three participants were run as a combination of diachronic and
...
the experienced errors in the computer game. Others (N=`n_noncomplaint`) were
...
solvers are affected by the size of their inheritance. Is future problem solving
unaffected by the number solutions inherited from previous generations? Finally,
we ask whether diachronic inheritance affects the type of guesses generated
by future generations of problem solvers. We compare the rates of redundant
to unique guesses among first generation problem solvers to future generations
of problem solvers.
...
innovations discovered by their ancestors (Fig.
\@ref(innovations-by-generation)). The benefit of inheritance decreased over
generations, such that later generations of problem solvers benefitted less from
inheritance than previous generations. Part of the reason for this is
independent of inheritance and may deal with the combinatorial complexity that
results from generating guesses from a larger inventory of items.
