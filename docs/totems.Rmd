...
There are multiple ways to assess performance. The performance measure shown here is the final number of inventions discovered by each team.

```{r performance-50}
```

```{r performance-fig, fig.height=6, fig.width=8, fig.cap="**Problem solving performance by strategy**. Left: The accumulation of inventions over labor time. Right: Final team performance."}
grid.arrange(
...
To compare problem solving efficiency between the three strategies, I first calculated guessing redundancy. The seemingly optimal strategy in this game is to never guess the same thing more than once, and yet participants routinely try the same (incorrect) guess over and over. This redundancy is a source of inefficiency; given the same number of guesses, a strategy that allows for less redundancy among its guessers will provide the greatest advantage in uncovering novel combinations of items.

I calculated two measures of redundancy: individual redundancy, which corresponds to the percentage of guesses that were duplicates of previous guesses, and team redundancy, which corresponds to the percentage of duplicate guesses in each team. Fig. 8 shows the results. Isolated individuals were the most redundant with themselves, and Synchronic teams were the most redundant with each other. Put differently, Diachronic collaboration was the least redundant strategy.

```{r redundancy-fig, fig.width=6, fig.cap="**Diachronic teamwork is less redundant than synchronic teamwork.** Diachronic teams are as likely to repeat themselves as isolated individuals, and also less likely to repeat their teammate than synchronic teams. Synchronic teams are less likely to repeat an incorrect guess they guessed previously, but more likely to repeat a guess made by their partner."}
redundancy_plot
```
...
```{r efficiency-fig, fig.width=6, fig.height=6}
efficiency_plot
```
...
```{r attempts-fig, fig.width=8, fig.cap="**Team attempts predict number of inventions.** Teams that make more attempts tend to discover more inventions. Since synchronic teams make more attempts, they in turn discover more inventions."}
grid.arrange(
  attempts_plot,
...
```{r guessing-rate, include=FALSE}
```

```{r guessing-rate-fig, fig.width=6}
grid.arrange(
  guessing_rate_over_time_plot,
  guessing_rate_plot,
  widths = c(0.6, 0.4)
)
```
...
```{r performance-100-fig, fig.height=6, fig.width=8, fig.cap="**Problem solving performance by strategy**. Left: The accumulation of inventions over labor time. Right: Final team performance."}
grid.arrange(
