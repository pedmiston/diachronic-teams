...
abstract: |
  Does inheriting an existing solution to a problem simply give a head start on solving it or does it actually change the way the problem is solved? The experiments reported here measure the impact of inheritance on problem solving ability by starting some problem solvers with a partial solution created by someone else to use as a starting point and testing whether they approach the problem differently than they would have if they would have reached the same point some other way. In the inheritance condition, one person works on a solution to a problem before passing it on to be inherited by the next person. Because the people in this exchange are working together, I refer to them as **Diachronic teams**. This research compares the problem solving abilities of Diachronic teams organized around inheritance to other ways of allocating the same number of labor hours: Synchronic teams working in parallel and Isolated individuals working for the same total amount of time. Comparing the relative efficiencies of these three strategies will reveal the impact of inheritance on problem solving.
csl: "apa.csl"
...
```{r fig4, fig.height=4, fig.width=8, fig.cap="**Problem solving performance by strategy**. The accumulation of inventions over time is shown on the left. On the right is final team performance. Synchronic collaboration is the most effective use of 50 labor minutes."}
grid.arrange(
