...
the number of innovations discovered by their predecessors
(Fig. \@ref(innovations-by-generation)). Second generation
participants tended to discover 
`r report_beta(innovations_by_generation_quad_mod, "Generation0")`
more innovations each generation,
`r report_lmer_mod(innovations_by_generation_quad_mod, "Generation0")`.
However, this linear effect was not constant over generations,
and decreased by `r report_beta(innovations_by_generation_mod, "Generation_2")` innovations each generation. Does this diminishing return indicate
that larger inheritances have a detrimental impact on problem
solving ability? We consider one alternative explanation before
testing this question directly.

(ref:innovations-by-generation) Number of innovations discovered by each generation. Each of the thin green lines is a team. Means in each generation are shown as green X's. The thick blue line is the model predictions with ±1 standard error.

```{r innovations-by-generation, fig.cap="(ref:innovations-by-generation)"}
innovations_by_generation_plot
```

The reason a slow down in problem solving rate is expected in this task is
that, as innovations are accumulated, the total number of possible guesses than
can be made from the accumulated items increases exponentially, so that later
generations of problems are likely to be more difficult even if the overall
problem solving rate has not changed. To control for combinatorial complexity,
we created an alternative outcome measure that, rather than counting each
innovation as equal, instead weighted each innovation relative to the size of
the inventory at the time at which the innovation was discovered. By this
measure, innovations from later generations are higher value because they
require combinatorially more guesses in order to find.

Using the weighted outcome measure, problem solvers were still found to
improve upon the innovations of their predecessors, `report_lmer_mod(difficulty_score_by_generations_mod, "Generation")`.
But the addition of a quadratic term did not significantly improve
model fit, `report_modcomp(difficulty_score_by_generation_modcomp)`,
suggesting that, when taking combinatorial complexity into account,
later generation problem solvers are not slowed down by a larger
inheritance.
...
To summarize, we find that participants are able to exceed the
discoveries of their ancestors through 4 generations of diachronic
problem solving. Later generation problem solvers do not
discover as many new solutions as earlier generations, but some
of this diminishing return is likely due to an increase in
combinatorial complexity as solutions are accumulated and used
again in future problems. Whether the number of inherited solutions
has an impact on future problem solving is tested in the next
section.
...
long to recreate the items discovered by their ancestor. These participants
...
To investigate the impact of the size of the inheritance on future problem solving, we look
specifically at the problems solved in the playing stage.
We found that larger inheritances are associated with fewer new innovations.
However, larger inheritances are also associated with less time for playing.
