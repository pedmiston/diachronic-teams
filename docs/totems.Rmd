...
On average, second generation diachronic participants took
`r exp1$mean_learning_time_min` minutes of a 25 minute session
(`r exp1$proportion_learning_time`%) to recreate the items that were discovered
by the previous generation. We refer to this stage of the experiment as the
learning stage.

The length of the learning stage depended on the number of items that were
inherited. On average second generation diachronic participants inherited
...
but there were some exceptions. A few participants took disproportionately
...
(ref:illustrative-diachronic-players) Problem solving with inheritance. Each
plot shows the rate of problem solving for a second generation diachronic
problem solver. Diachronic problem solvers spent the first stage of the
experiment recreating the discoveries made by the previous generation. After
this learning stage, diachronic problem solvers entered the discovery stage,
where they attempted to discover new items. The left three plots show trajectories
for individual participants. The rightmost plot shows the trajectories for all
participants centered on the time at which the learning stage was completed.

```{r illustrative-diachronic-players, fig.width=8, fig.height=3, fig.cap="(ref:illustrative-diachronic-players)"}
grid.arrange(
...
In the first experiment we compared the effectiveness of the simplest form
