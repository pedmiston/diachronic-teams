...
abstract: |
  ...
  a potential downstream consquence to iterated problem solving. We discuss the
...
amount of time than their ancestors (Fig. \@ref(fig:innovations-by-generation)).
We tested this in multiple ways. First, we used Page's trend test (a repeated
measures test for monotonicity), and found evidence that the number of tools
discovered within each chain increased over generations `r exp1$page_test`.
Next, we tested whether the extent to which participants were able to exceed
their ancestors changed over generation. We fit a hierarchical regression
model to the number of tools discovered in each generation with these effects
nested by chain. On average, second generation participants were able to
...
`r exp1$gen0sqr_slope_stats`. This diminishing return suggests that future
generations have a harder time exceeding their ancestors.
...
of tools that were inherited (Fig. \@ref(fig:inheritance-size)). As the number
of inherited tools increased, the number of new tools discovered decreased,
`r exp1$inheritance_size_slope_stats` (Fig. \@ref(fig:inheritance-size)B).
That participants who inherited more items had a harder time exceeding them
is expected based on the analysis of performance over generations reported
above. However, when tools are weighted by combinatorial complexity, the
amount by which participants are able to exceed their ancestors actually
increases for larger inheritance sizes, `r exp1$delta_diff_slope_stats` (Fig.
\@ref(fig:inheritance-size)C), though we are hesitant to draw strong conclusions
from this finding as it may be explanable by the characteristics of the solution
landscape, a point we return to in the Discussion.
...
session spent discovering new tools (Fig. \@ref(fig:playing-time)). The overall rate of
...
item, `r exp1$guesses_per_new_item_by_inheritance_mod` (Fig.
\@ref(fig:guesses-per-innovation-1)). This finding suggests that although inheritance
benefits participants in recreating inherited solutions, it does not confer any
benefits to future problem solving.
...
Fig. \@ref(fig:guess-types-1). We found that participants who inherited solutions at the start
...
```{r guess-types-1, fig.width=4, fig.cap="Guessing strategies by inheritance condition. Redundant guesses were incorrect guesses made once before. Repeat items were recreated tools. Unique guesses were incorrect guesses never made. Unique items were unique guesses that created a new tool."}
