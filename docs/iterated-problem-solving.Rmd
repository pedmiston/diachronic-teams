...
[@Caldwell:2008gf]. However, the conditions necessary for cumulative gains
remain uncertain. In familiar and transparent tasks like building paper
airplanes, process information about how the planes were built may not be
necessary for obtaining cumulative gains [@Caldwell:2009fl], but these gains
may be similar to what could be achieved by individual learners
[@Zwirner:2015jn]. In unfamiliar and opaque tasks, social learners benefit from
process information [@Derex:2013ia; @Wasielewski:2014gg], but may not show
cumulative gains [@Morgan:2015hu]. Here we controlled for successful
transmission by allowing future generations to inherit symbolic information
("recipes") about how to recreate the tools that had been discovered by an
ancestor. We then measured the ability of participants to recreate and exceed
...
than others, indicating they are using previously acquired knowledge about the tools to
...
to generation). In the second section, we also quantify the amount of time each
...
```{r innovations-by-generation, fig.cap="Tools by generation. Each of the thin green lines is a chain. The thick blue line is the model predictions with ±1 standard error."}
...
from anyone (No inheritance) to participants in generations 2-4 who inherited
at least some tools from an ancestor (Inheritance). We compared these two
groups in terms of the number of guesses required to discover each tool.
...
based on inheritance condition (No inheritance, Inheritance) with random
effects by tool. Tools that were only discovered by participants in the
inheritance condition were excluded from this analysis.

For those in the inheritance condition, we separated our analyses based on
whether the tool was inherited from an ancestor or discovered new. This allowed
us to test whether the benefit to inheritance was restricted to reducing the
number of guesses for inherited items, or whether inheritance had any effect
on future guessing behaviors.

As expected, participants who benefited from inheritance on average
made fewer guesses per tool than when the same tools were attempted in the
first generation, `r exp1$guesses_per_item_by_inheritance`. This effect
demonstrates the benefit of inheriting from a previous generation in providing
a shortcut to discovering these tools.
...
```{r guesses-per-innovation-1, fig.height=6, fig.cap="Guesses per tool by inheritance condition. Each line is the average number of guesses it took to discover a particular tool in either inheritance condition. Error bars show ±1 standard of the model predictions. A. Inherited tools. B. Novel tools."}
grid.arrange(
