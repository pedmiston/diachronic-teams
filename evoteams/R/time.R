...
#' Create a ggplot showing the discrepancy between calendar
#' hours and labor hours for diachronic and synchronic teams.
#'
#' @import dplyr
#' @import ggplot2
#' @export
make_hours_per_team_plot <- function(team_colors, line_size) {
  if (missing(team_colors)) team_colors <- get_team_colors()
  if (missing(line_size)) line_size <- 1.5

  time <- make_time_data()
  time_plot <- make_time_plot()
...
# Currently unused:
# make_hours_for_multiple_projects_plot <- function() {
#   time_plot <- make_time_plot()
...
#' Make up some data used to show the time discrepancy between team structures.
#' @import dplyr
#' @export
make_time_data <- function() {
  diachronic <- dplyr::data_frame(
...
#' Make a base plot showing relationship between calendar and labor hours.
#' @import ggplot2
#' @export
