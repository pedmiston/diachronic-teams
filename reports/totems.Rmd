...
time <- rbind(diachronic, synchronic, isolated) %>%
  group_by(Strategy, Player) %>%
  mutate(PlayerHours = 1:n()) %>%
  ungroup() %>%
  recode_strategy()
...
gg_player <- ggplot(time) +
  aes(StrategyIsolated, PlayerHours) +
  geom_bar(aes(fill = StrategyLabel), stat = "summary", fun.y = "max") +
  scale_x_discrete("") +
  scale_y_continuous("Player hours", breaks = axis_breaks, labels = axis_labels) +
  scale_fill_strategy +
  base_theme +
