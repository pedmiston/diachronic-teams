...
What individuals can do and learn to do today critically depends on what others were able to do yesterday. Humans are smart, but we are also hoarders; human cultures accumulate artifacts, ideas, and information far more than any other animal[^uniqueness]. Part of what makes this accumulation helpful is that people never have to start from scratch when solving a problem. Inventors, for example, inherit and adapt existing solutions to problems so pervasively as to make the history of technology one of near continuous change, with each new invention depending heavily on its precursors [@Basalla:1989wq]. But what this historical evidence does not provide is a way to understand more directly the role inheritance may play in the problem solving abilities of individuals. Does inheriting a partial solution just give people a head start or does it actually make them more effective problem solvers? The experiments below are designed to test and measure the impact of inheritance on the problem solving abilities of individuals by studying their productivity in teams. In the control condition, three person teams are given two hours in the same room to solve a problem. Because the members of the team are all working at the same time, these are called **synchronic teams**. In the experimental condition, teamwork is not synchronous and is instead organized around inheritance over time. These **diachronic teams** work one at a time, each (other than the first) inheriting as a starting point the previous team member's solution. My hypothesis is that organizing teamwork diachronically will make for more effective problem solving. These results would explain how the tools and abilities that facilitate inheritance and diachronic collaboration may serve to bootstrap the problem solving abilities of humans.
...
```{r team-structure, engine = "dot", fig.cap = "Team structures. Teams of three are allotted the same number of labor hours to solve a problem. Synchronic teams work all at the same time on a single solution. Diachronic teams work one at a time, each (other than the first) inheriting the previous solution and improving it. The extent to which diachronic teamwork is more effective than synchronic teamwork is proposed as a test of the role of inheritance in the problem solving ability of individuals."}
```

The conclusions that can be drawn from these experiments depend heavily on the problem being solved. Obviously there are many problems that cannot be accomplished by three-person teams each working for two hours, and many more that are not difficult enough to warrant teamwork. In addition to being the right level of difficulty, the problems must also allow for inheritance of solutions. The final requirement is that there are objective measures of solution success. These requirements enable the investigation of the role of inheritance in problem solving ability.

Why might inheritance lead to more effective problem solving? One potential explanation is that inheriting an existing solution promotes responsiveness to feedback. If this is true, then altering the feedback available to teams should differentially impact diachronic and synchronic teams. What these results would mean for our understanding of cultural evolution is that inheritance may have the strongest impact on problem solving ability for problems where there are informative sources of feedback.
