...
What individuals can do and learn to do today critically depends on what others were able to do yesterday. Humans are smart, but we are also hoarders; human cultures accumulate artifacts, ideas, and information far more than any other animal[^uniqueness]. Part of what makes this accumulation helpful is that people never have to start from scratch when solving a problem. Inventors, for example, inherit and adapt existing solutions to problems so pervasively as to make the history of technology one of near continuous change, with each new invention depending heavily on its precursors [@Basalla:1989wq]. But what this historical evidence does not provide is a way to understand more directly the role inheritance may play in the problem solving abilities of individuals. Does inheriting a partial solution actually make individuals more effective problem solvers or does it simply give people a head start? The experiments below are designed to test and measure the impact of inheritance on the problem solving abilities of individuals working together in teams. In the control condition, three person teams are given two hours in the same room to solve a problem. Because they are working at the same time, these teams are called **synchronic teams**, as opposed to **diachronic teams**, which is the experimental condition. Diachronic teams work one at a time, each inheriting as a starting point the previous team member's solution. My hypothesis is that arranging teamwork diachronically will make for more effective problem solving when there are reliable sources of feedback.

[^uniqueness]: Some elements of cumulative culture appear in other animals. See @Hunt:2003dj for an investigation of inherited variation in tool types among New Caledonian crows.

```{r team-structure, engine = "dot", fig.cap = "Team structures. Teams of three are allotted the same number of labor hours to solve a problem. Synchronic teams work all at the same time on a single solution. Diachronic teams work one at a time, each inheriting the previous solution and improving it. Comparing the solution success of diachronic and synchronic teams reveals the role of inheritance in the problem solving abilities of individuals."}
```



The conclusions that can be drawn from these experiments depend heavily on the type of problem being solved. In order to be an informative experiment, the problems must be hard enough to benefit from three-person teamwork but not so hard as to prevent contributions from all team members. These conditions can be met by verifying the difficulty of the problems with additional control groups. Other than being the right level of difficulty, the problem being solved also must allow for many possible solutions all varying in degree of success. The reason is that inherited solutions are only as good as their quality. As a simple example,
...
```{r all-teams, engine = "dot", fig.width = 6, fig.cap = "Team structures. Teams of three are allotted the same number of labor hours to solve a problem. In the control condition, team members work all at the same time. They can discuss and divide labor any way they choose. Because the team is working all at the same time, these are called **synchronic teams**, to be contrasted with **diachronic teams**, the experimental condition. In diachronic teams, team members work one at a time, and each person after the first starts with (or inherits) the previous team member's solution. The extent to which diachronic teams are more effective problem solvers than synchronic teams is taken as evidence of the role of inheritance in the problem solving abilities of individuals. **Solo** \"teams\" work alone for 6 hours in 2 hour increments. **Expensive** teams are given 3 times as many labor hours as diachronic and synchronic teams. The performance of solo teams, compared to diachronic and synchronic team performance, is used to verify that the problems are difficult enough to warrant teamwork. The performance of expensive teams is used to estimate best possible performance of diachronic and synchronic teams."}
```
...
```{r multiple-projects, engine = "dot", fig.cap = "Two ways of allocating the same number of calendar hours toward multiple projects. In synchronic teams, team members work on each project from beginning to end in lockstep. In diachronic teams, team members work on different projects at the same time, and collaborate over time."}
```

**Procedure**. Experiment 4 is conducted like Experiment 1, except both teams now compete for 8 hours.
