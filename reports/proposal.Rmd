...
What individuals can do and learn to do today critically depends on what others were able to do yesterday. Humans are smart, but we are also hoarders; human cultures accumulate artifacts, ideas, and information far more than any other animal [but see @Hunt:2003dj]. Part of what makes this accumulation helpful is that people never have to start from scratch when solving a problem. Inventors, for example, inherit and adapt existing solutions to problems so pervasively as to make the history of technology one of near continuous change, with each new invention depending heavily on its precursors [@Basalla:1989wq]. But what this historical evidence does not provide is a way to understand more directly the role inheritance may play in the problem solving abilities of individuals. The experiments below are designed to test and measure the impact of inheritance on the problem solving abilities of individuals working together in teams. In the control condition, three person teams are given two hours in the same room to solve a problem. In the experimental condition, teams work one at a time, each inheriting as a starting point the previous team member's solution. The effect of inheritance on problem solving ability is measured by improvement in solution success per labor hour when a starting solution is inherited compared to when it must be created.

```{r team-structure, engine = "dot"}
```

Much depends on the type of problem being solved. In order to be an informative experiment, the problems must be hard enough to benefit from teamwork but not so hard as to prevent contributions from all team members. These conditions can be met by recruiting skilled and motivated participants, and verifying the difficulty of the problems with additional control groups. Under these conditions, where teamwork is helpful but not essential to problem solving, the role of inheritance on the problem solving ability of individuals can be measured.

```{r control-groups, engine = "dot"}
```
